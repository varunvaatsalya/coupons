...
export async function GET(req) {
  ...
  try {
    const [merchantType, merchantStatus, geographicMarket, currency] =
      await Promise.all([
        prisma.merchantType.findMany({ orderBy: { name: "asc" } }),
        prisma.merchantStatus.findMany({ orderBy: { name: "asc" } }),
        prisma.geographicMarket.findMany({ orderBy: { name: "asc" } }),
        prisma.currency.findMany({ orderBy: { code: "asc" } }),
      ]);

    return NextResponse.json(
      {
        data: { merchantType, merchantStatus, geographicMarket, currency },
        success: true,
      },
      { status: 200 }
    );
...
const modelMap = {
  ...
  merchantStatus: prisma.merchantStatus,
  geographicMarket: prisma.geographicMarket,
